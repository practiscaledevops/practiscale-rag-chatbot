...
-- ===========================================================================
--  USER PROMPT LIBRARY (from migration 0004)
-- ===========================================================================
-- Per-user reusable prompts, RLS-scoped to the owner. Idempotent.
...
begin
  if not exists (select 1 from pg_policies where tablename = 'user_prompts' and policyname = 'user_prompts_select_own') then
    create policy "user_prompts_select_own" on public.user_prompts for select using (auth.uid() = user_id);
  end if;
  if not exists (select 1 from pg_policies where tablename = 'user_prompts' and policyname = 'user_prompts_insert_own') then
    create policy "user_prompts_insert_own" on public.user_prompts for insert with check (auth.uid() = user_id);
  end if;
  if not exists (select 1 from pg_policies where tablename = 'user_prompts' and policyname = 'user_prompts_update_own') then
    create policy "user_prompts_update_own" on public.user_prompts for update using (auth.uid() = user_id) with check (auth.uid() = user_id);
  end if;
  if not exists (select 1 from pg_policies where tablename = 'user_prompts' and policyname = 'user_prompts_delete_own') then
    create policy "user_prompts_delete_own" on public.user_prompts for delete using (auth.uid() = user_id);
  end if;
end $$;
